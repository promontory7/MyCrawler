...
public class CrawlerTask {
	public static void startCrawlerTask() {
		...
		Spider.create(new ShanTouProcessor()).addUrl(ShanTouProcessor.url).thread(1).run();
		...
		Spider.create(new ZhaoqingPrecessor()).addUrl(ZhaoqingPrecessor.url).thread(5).run();
		...
		Spider.create(new ShaoguangProcessor()).addUrl(ShaoguangProcessor.url).thread(1).run();
